...
public class PlanningModule : MonoBehaviour
{
    ...
    private IEnumerator RunLLM3(PlanSlot[] availableSlots)
    {
        ...
        string prompt =
            ...
            "1. 输出内容仅包含 JSON。\n" +
            "2. `slotId` 是可选槽中的一个。\n" +
            "3. `thought` 按如下结构：选槽理由（电量/位置/槽可用性/角色匹配） → 【置信】对此选择的把握（高/中/低）及原因 → 【建议】若有值得记录的规律输出\"当[场景]时应[策略]\"，否则留空。\n\n" +
            "输出格式示例:\n" +
            "{\n" +
            "  \"thought\": \"当前电量充足且位置靠近南区，s0(Scout)最适合 → 【置信】高，无竞争槽位 → 【建议】当电量低于30%时应优先选择距充电点近的槽位\",\n" +
            "  \"slotId\": \"s0\"\n" +
            "}";
