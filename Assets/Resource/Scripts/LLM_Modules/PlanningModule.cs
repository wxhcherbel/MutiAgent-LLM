...
public class PlanningModule : MonoBehaviour
{
    ...
    private IEnumerator RunLLM1(string desc, int cnt)
    {
        ...
        string prompt =
            ...
            "{\n" +
            "  \"thought\": \"A/B各负责独立区域→C1；充电桩同一时刻只能一架使用→C3互斥；均完成后统一回传→C2同步\",\n" +
            "  \"relType\": \"Cooperation\",\n" +
            "  \"groupCnt\": 1,\n" +
    ...
    private IEnumerator RunLLM2()
    {
        ...
        string prompt =
            ...
            "输出格式:\n" +
            "{\n" +
            "  \"thought\": \"一句话说明角色设计的关键依据（如何根据任务分工和约束决定槽数量与角色）\",\n" +
            "  \"slots\": [PlanSlot, ...],\n" +
            ...
            "输出:\n" +
            "{\n" +
            "  \"thought\": \"C1约束指定A负责南区、B负责北区，设2个Scout槽分工明确；C3+1要求s0先行发信号，s1等待；C3-1互斥充电桩两槽都绑定\",\n" +
            "  \"slots\": [\n" +
    ...
    private IEnumerator RunLLM3(PlanSlot[] availableSlots)
    {
        ...
        string prompt =
            ...
            "1. 输出内容仅包含 JSON。\n" +
            "2. `slotId` 是可选槽中的一个。\n" +
            "3. `thought` 简要说明选择该槽的理由（电量/位置/槽可用性/角色匹配等）。\n\n" +
            "输出格式示例:\n" +
            "{\n" +
            "  \"thought\": \"当前电量充足且位置靠近南区，s0(Scout)最适合\",\n" +
            "  \"slotId\": \"s0\"\n" +
            "}";
